...
#define HIP_CHECK(x) do { hipError_t e=(x); if(e!=hipSuccess){fprintf(stderr,"HIP %d %s:%d\n",e,__FILE__,__LINE__);exit(1);}} while(0)

using namespace mori::core;

// GPU kernel: submit N RDMA writes via proxy ring
__global__ void gpu_proxy_write_kernel(
    volatile ProxyRing* ring, uint32_t qp_idx,
    uint64_t src, uint32_t lkey, uint64_t dst, uint32_t rkey,
...
__global__ void gpu_proxy_write_inline_kernel(
    volatile ProxyRing* ring, uint32_t qp_idx,
    uint64_t src, uint32_t lkey, uint64_t dst, uint32_t rkey,
    uint32_t xfer_size, int num_ops, volatile int* result) {
  if (threadIdx.x || blockIdx.x) return;
...
__global__ void gpu_proxy_multi_warp_kernel(
    volatile ProxyRing* ring, uint32_t qp_idx,
    uint64_t src, uint32_t lkey, uint64_t dst, uint32_t rkey,
...
struct TestCtx {
  ibv_context* ctx; ibv_pd* pd; ibv_cq* cq; ibv_qp* qp; ibv_mr* mr;
  void* gpu_buf; size_t buf_size;
};

static TestCtx setup_loopback(const char* dev_name, int gid_idx) {
  TestCtx t{};
  int nd; ibv_device** dl = ibv_get_device_list(&nd); ibv_device* d = nullptr;
  for (int i = 0; i < nd; i++) if (!strcmp(dl[i]->name, dev_name)) d = dl[i];
  assert(d);
  t.ctx = ibv_open_device(d); t.pd = ibv_alloc_pd(t.ctx);
  t.cq = ibv_create_cq(t.ctx, 256, nullptr, nullptr, 0);
  ibv_qp_init_attr qa{}; qa.send_cq = t.cq; qa.recv_cq = t.cq; qa.qp_type = IBV_QPT_RC;
  qa.cap = {128, 128, 1, 1, 0}; t.qp = ibv_create_qp(t.pd, &qa);
  t.buf_size = 64 * 1024;
  ...
  assert(t.mr);
  ibv_gid gid; ibv_query_gid(t.ctx, 1, gid_idx, &gid);
...
int main(int argc, char** argv) {
  const char* dev = "ionic_0"; int gid = 1;
  ...
  *result = 0;
  hipLaunchKernelGGL(gpu_proxy_write_kernel, dim3(1), dim3(1), 0, 0,
      (volatile ProxyRing*)ring, 0,
      (uint64_t)t.gpu_buf, t.mr->lkey,
      (uint64_t)t.gpu_buf + 4096, t.mr->rkey,
      4096u, 10, result);
  ...
  *result = 0;
  ...
  hipLaunchKernelGGL(gpu_proxy_write_kernel, dim3(1), dim3(1), 0, 0,
      ...
      (uint64_t)t.gpu_buf, t.mr->lkey,
      (uint64_t)t.gpu_buf + 4096, t.mr->rkey,
      4096u, num_ops, result);
  HIP_CHECK(hipDeviceSynchronize());
  auto t1 = std::chrono::high_resolution_clock::now();
  double us = std::chrono::duration<double, std::micro>(t1 - t0).count();
  printf("  Test 4: %d ops in %.1f ms = %.0f ops/s, %.2f GB/s, %.1f us/op %s\n",
         *result, us / 1e3, num_ops / (us / 1e6),
         (double)num_ops * 4096 / (us / 1e6) / 1e9, us / num_ops,
         *result == num_ops ? "PASS" : "FAIL");
  assert(*result == num_ops);

  // Cleanup
  proxy.Shutdown();
  ibv_destroy_qp(t.qp); ibv_destroy_cq(t.cq); ibv_dereg_mr(t.mr);
  hipFree(t.gpu_buf); hipHostFree(ring); hipHostFree(result);
  ibv_dealloc_pd(t.pd); ibv_close_device(t.ctx);
